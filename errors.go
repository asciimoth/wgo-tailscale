...
var (
	...
	ErrDeviceRequired         = errors.New("tailscale: wgo device API required")
	ErrDeviceAlreadyAttached  = errors.New("tailscale: wgo device API already attached")
	ErrZeroNodeKey            = errors.New("tailscale: wgo device has no private node key")
	ErrNodeIdentityChanged    = errors.New("tailscale: wgo device identity does not match the client identity")
